import { Profesional } from "./interface/ProfessionalInteface";

export const profesionalData: Profesional[] = [
  {
    cordinadora: true,
    nombre: "Lic. Silvina Vázquez",
    imagen: "/silvina.jpeg",
    universidad: "Universidad de Buenos Aires (UBA)",
    especialidades: [
      "Ex Inspectora PCyPS - DGCyE",
      "Capacitadora Docente CIIE",
      "Psicoterapia de Jóvenes, Adultos y Parejas. ",
    ],
    curriculum:
      "https://drive.google.com/file/d/1Dqx5Ry_l2w_UZFCrFqVaiQkpgZsmCyK3/view",
  },
  {
    cordinadora: true,
    nombre: "Lic. Lilians Dotti Quintana",
    imagen: "/lilians.jpeg",
    universidad: "Universidad de la República Oriental del Uruguay (UdelaR)",
    especialidades: [
      "Docente de nivel Universitario y Secundario",
      "Modelo Sistémico",
      "Formación en grupos, especialista en terapia sistémica",
      "orientación vocacional y proyecto de vida",
    ],
    curriculum: 'https://drive.google.com/file/d/1WEWX-GBek3TdPlwzY_jtq5Dcwo5N03nF/view?usp=sharing'
  },
  {
    cordinadora: false,
    nombre: "Lic. Rocío Palacio",
    imagen: "/psicologa1.jpg",
    universidad: "Universidad de Buenos Aires (UBA)",
    especialidades: [
      "Psicodiagnóstico",
      "Orientación Vocacional",
      "Especialista en psicoterapia para adolescentes y adultos",
    ],
    curriculum:
      "https://drive.google.com/file/d/1AIxYmCLD04azCnA-MZJSJSoq2n8ugI8d/view",
  },
  {
    cordinadora: false,
    nombre: "Lic. Sonia de Leon",
    imagen: "/psicologa1.jpg",
    universidad: "Facultad de Psicología, UBA.",
    especialidades: [
      "Psicóloga clínica con orientación cognitivo-conductual",
      "acompañamiento terapéutico de adolescentes y adultos",
      " enfoque basado en la evidencia científica",
    ],
    curriculum: 'https://drive.google.com/file/d/12NrI-wwXm4p49q_5jCOVQtG4fhKwNzqG/view?usp=sharing'
  },
      {
    cordinadora: false,
    nombre: "Lic. Julio Molina Debat.",
    imagen: "https://res.cloudinary.com/dc8aa3b1b/image/upload/v1758561671/WhatsApp_Image_2025-09-20_at_10.56.08_AM_flr6mw.jpg",
    universidad: "Facultad de Psicología, Universidad de la República, Uruguay.",
    especialidades: [
      " Psicología del Deporte y la Actividad Física",
      "Trastorno del Espectro Autista e Inclusión Educativa",
      "Especialización en TDAH",
    ],
    curriculum: 'https://docs.google.com/document/d/1HUW8G25CLICiOwyldQSNwVbVOmYW1_oN/edit?usp=sharing&ouid=109163468251593264356&rtpof=true&sd=true'
  },
       {
    cordinadora: false,
    nombre: "Lic. Silvia Mónica Fernández.",
    imagen: "https://res.cloudinary.com/dc8aa3b1b/image/upload/v1758561793/WhatsApp_Image_2025-09-22_at_10.22.25_AM_kh1ehv.jpg",
    universidad: "Universidad de Buenos Aires (UBA)",
    especialidades: [
      " Psicoterapia de Jóvenes, Adultos, Pareja. ",
      "Especialista en Terapia Familiar Sistémica.",
    ],
    curriculum: 'https://docs.google.com/document/d/1HUW8G25CLICiOwyldQSNwVbVOmYW1_oN/edit?usp=sharing&ouid=109163468251593264356&rtpof=true&sd=true'
  },
];
